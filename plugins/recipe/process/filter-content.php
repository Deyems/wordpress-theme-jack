...
function r_filter_recipe_content($content){
    ...
    $recipe_html = str_replace('ORIGIN_118N', __('Origin','recipe'), $recipe_html);
    $recipe_html = str_replace('ORIGIN_PH', isset($origin[0]) ? $origin[0]->name : 'None', $recipe_html);

    if(!empty($more_info_url)){
        $recipe_html = str_replace("_MORE_INFO_URL_PH", '<a href="'. $more_info_url .'"> More info </a>',$recipe_html);
    }else{
        $recipe_html = str_replace("_MORE_INFO_URL_PH", '', $recipe_html);
    }
